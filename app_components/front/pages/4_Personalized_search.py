...
if cust_id:
    ...
    if not result.empty:
        ...
        not_found_bg = "#5a2b2b" if mode == "Dark Mode" else "#ffecec"
        not_found_text = "#f0f0f0" if mode == "Dark Mode" else "#b30000"
        st.markdown(f"""
        <div style='background-color: {not_found_bg}; padding: 10px; border-radius: 8px; margin-top:10px;'>
            <h4 style='color: {not_found_text}; margin:0;'>No customer found.</h4>
        </div>
        """, unsafe_allow_html=True)
...
if mode == "Light Mode":
    st.markdown("""
        ...
        input[type="text"] {border: 1px solid #00b4d8; border-radius: 8px; padding: 10px; background-color: #f0f9fa; color: #006d77; caret-color: black;}
        input[type="text"]:focus {border: 1px solid #00b4d8; background-color: #ffffff;}
    ...
    st.markdown("""
        ...
        input[type="text"] {border: 1px solid #00f0ff; border-radius: 8px; padding: 10px; background-color: #002222; color: #f0f0f0; caret-color: white;}
        input[type="text"]:focus {border: 1px solid #00f0ff; background-color: #001c1c;}
